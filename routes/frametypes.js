...
router.post('/insertframe',upload.single("adpicture"), function(req, res, next) {
  console.log("BODY:", req.body);
  console.log("FILE", req.file);
  pool.query("insert into frametypes (framename,adpicture,status) values(?,?,?)",
  [req.body.framename,req.body.myfilename,req.body.status],function(error,result){
      if(error)
      { console.log(error)
